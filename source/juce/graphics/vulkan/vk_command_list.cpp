...
#include "experimental/vk_transform.h"
#include "vk_render_target.h"

namespace juce
{

vk_command_list::vk_command_list(VkDevice device, VkCommandPool pool, VkCommandBufferLevel level) :
    m_device(device), m_pool(pool)
{
	...
	VK(vkAllocateCommandBuffers(m_device, &info, &m_handle));
}

vk_command_list::~vk_command_list()
{
	if(!valid())
		return;
...
void vk_command_list::convert_vk_resource_layout(const resource_layout& resouce_state,
                                                 vk_resouce_state&      states)
{
	switch(resouce_state) {
		...
		case resource_layout::undefined:
		{
			states.stage  = VK_PIPELINE_STAGE_2_NONE;
			states.acess  = 0;
...
void vk_command_list::resouce_barrier(const image_transition& trans)
{
	...
	convert_vk_resource_layout(trans.before, src);
	convert_vk_resource_layout(trans.after, dst);
