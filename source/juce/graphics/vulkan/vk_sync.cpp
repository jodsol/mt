...
namespace juce
{
vk_sync::vk_sync(VkDevice device) :
    m_device(device)
{
	create_sync_objects();
}
...
VkSemaphore vk_sync::get_image_available_semaphore(uint32_t frame_index) const
{
	return m_image_available_semaphores[frame_index];
}

VkSemaphore vk_sync::get_render_finished_semaphore(uint32_t frame_index) const
{
	return m_render_finished_semaphores[frame_index];
}

VkFence vk_sync::get_inflight_fence(uint32_t frame_index) const
{
	return m_inflight_fences[frame_index];
}

void vk_sync::wait_for_fence(uint32_t frame_index)
{
	VkFence fence = get_inflight_fence(frame_index);
	vkWaitForFences(m_device, 1, &fence, VK_TRUE, UINT_MAX);
}

void vk_sync::reset_fence(uint32_t frame_index)
{
	// VkFence fence = get_inflight_fence(frame_index);
	vkResetFences(m_device, 1, &m_inflight_fences[frame_index]);
}

void vk_sync::create_sync_objects()
{
...
void vk_sync::destroy_sync_objects()
{
	for(size_t i = 0; i < m_max_frames_inflight; ++i) {
